...
# main class of implementing the pullup counter
class PullupCounter:
    ...
    def __init__(self, file_name, activate_coef, buffer_time):
        ...
        # landmark indices for elbows
        self.selected_landmarks_3 = [13, 14]
        self.selected_landmarks_rtmlib_3 = [7, 8]
        # landmark indices for hands
        self.selected_landmarks_4 = [15, 16, 17, 18, 19, 20, 21, 22]
        self.selected_landmarks_rtmlib_4 = [9, 10]
        # landmark indices for feet
        self.selected_landmarks_5 = [29, 30, 31, 32]
        self.selected_landmarks_rtmlib_5 = [15, 16]
        # some default values
        self.cy_max = 100
        self.cy_min = 100
        self.cy_eh_max = 100
        ...
        self.cy_eh_buffer = BufferList(buffer_time)
        self.cy_eh_up = BufferList(buffer_time)
        self.cy_eh_down = BufferList(buffer_time)
        self.cy_hf_buffer = BufferList(buffer_time)
        self.cy_hf_up = BufferList(buffer_time)
        self.cy_hf_down = BufferList(buffer_time)
        ...
        self.landmark_3_buffer = BufferList(buffer_time)
        self.landmark_4_buffer = BufferList(buffer_time)
        self.landmark_5_buffer = BufferList(buffer_time)
        self.shoulder_hip_buffer = BufferList(buffer_time)
        self.elbow_hand_buffer = BufferList(buffer_time)
        ...
        self.critirion_2 = BufferList(buffer_time)
    ...
    def count_plot(self, display=True):
        ...
        cy_max = self.cy_max
        cy_min = self.cy_min
        cy_eh_max = self.cy_eh_max
        cy_eh_min = self.cy_eh_min
        cy_hf_max = self.cy_hf_max
        cy_hf_min = self.cy_hf_min
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened():
                ...
                if results.pose_landmarks:
                    ...
                    cx_2 = int(np.mean([x[0] for x in landmarks_2]))
                    ...
                    # x, y coordinates for elbows
                    landmarks_3 = [
                        (lm.x * image_width, lm.y * image_height)
                        for i, lm in enumerate(results.pose_landmarks.landmark)
                        if i in self.selected_landmarks_3
                    ]
                    # mean coordinates for elbows
                    cy_3 = int(np.mean([x[1] for x in landmarks_3]))
                    self.landmark_3_buffer.push(cy_3)

                    # x, y coordinates for hands
                    landmarks_4 = [
                        (lm.x * image_width, lm.y * image_height)
                        for i, lm in enumerate(results.pose_landmarks.landmark)
                        if i in self.selected_landmarks_4
                    ]
                    # mean coordinates for hands
                    cy_4 = int(np.mean([x[1] for x in landmarks_4]))
                    self.landmark_4_buffer.push(cy_4)
                    cy_elbow_hand = cy_3 - cy_4
                    self.elbow_hand_buffer.push(cy_elbow_hand)

                    # x, y coordinates for feet
                    landmarks_5 = [
                        (lm.x * image_width, lm.y * image_height)
                        for i, lm in enumerate(results.pose_landmarks.landmark)
                        if i in self.selected_landmarks_5
                    ]
                    # mean coordinates for feet
                    cy_5 = int(np.mean([x[1] for x in landmarks_5]))
                    self.landmark_5_buffer.push(cy_5)
                    cy_hand_feet = cy_5 - cy_4

                else:
                    cy_1 = 0
                    cy_shoulder_hip = 0

                # center coordinate of shoulder position
                cy = int((cy_2 + self.center_y.buffer[-1]) / 2)
                # set data
                self.center_y.push(cy)

                # upper bound for center coordinate of shoulder position
                cy_max = 0.5 * cy_max + 0.5 * self.center_y.max()
                self.center_y_up.push(cy_max)

                # lower bound for center coordinate of shoulder position
                cy_min = 0.5 * cy_min + 0.5 * self.center_y.min()
                self.center_y_down.push(cy_min)

                # vertical distance between elbows and hands
                cy_eh = int((cy_elbow_hand + self.cy_eh_buffer.buffer[-1]) / 2)
                # set data
                self.cy_eh_buffer.push(cy_eh)

                # upper bound for vertical distance between elbows and hands
                cy_eh_max = 0.5 * cy_eh_max + 0.5 * self.cy_eh_buffer.max()
                self.cy_eh_up.push(cy_eh_max)

                # lower bound for vertical distance between elbows and hands
                cy_eh_min = 0.5 * cy_eh_min + 0.5 * self.cy_eh_buffer.min()
                self.cy_eh_down.push(cy_eh_min)

                # vertical distance between hands and feet
                cy_hf = int((cy_hand_feet + self.cy_hf_buffer.buffer[-1]) / 2)
                # set data
                self.cy_hf_buffer.push(cy_hf)

                # upper bound for vertical distance between elbows and hands
                cy_hf_max = 0.5 * cy_hf_max + 0.5 * self.cy_hf_buffer.max()
                self.cy_hf_up.push(cy_hf_max)

                # lower bound for vertical distance between elbows and hands
                cy_hf_min = 0.5 * cy_hf_min + 0.5 * self.cy_hf_buffer.min()
                self.cy_hf_down.push(cy_hf_min)
                ...
                dy = cy_max - cy_min
                dy_eh = cy_eh_max - cy_eh_min

                # two criteria for detecting a sub pullup process
                self.bound_switch_up.push(cy_min + 0.35 * dy)
                self.bound_switch_down.push(cy_max - 0.55 * dy)

                # detect whether the person is grabbing the bar
                self.critirion_1.push(cy_hf_max - 2.5*dy)
                # detect whether the pullup amplitude is large enough for the person
                self.critirion_2.push(dy - dy_eh)
                if cy_hf_max > 2.5*dy and dy > dy_eh:
                    # current is up pose
                ...
                if flip_flag < prev_flip_flag:
                    ...
                    # update the count if there is a period of pullup
                    count = count + 1
                ...
                if frame_num <= self.buffer_time:
                    ...
                    cv2.circle(image, (cx_2, cy), 5, (0, 0, 255), -1)
                    cv2.putText(
                        image,
                        "centroid",
                        (cx_2 - 25, cy - 25),
                    ...
                    if cy_hf_max <= 2.5*dy:
                        cv2.putText(
                            image,
                            "Warning: Not in desirable pose!",
                            (int(image_width * 0.6), int(image_height * 0.4)+60),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            1,
                            (0, 255, 0),
                            3,
                ...
                if display:
                    ...
                    plt.plot(self.cy_eh_buffer.buffer, label="cy_elbow_hand")
                    ...
                    plt.plot(self.landmark_3_buffer.buffer, label="landmark_3")
                    plt.plot(self.landmark_4_buffer.buffer, label="landmark_4")
                    ...
                    plt.plot(self.critirion_2.buffer, label="critirion_1")
                    plt.plot(self.critirion_2.buffer, label="critirion_2")
    ...
    def count_plot_rtmlib(self, pose='local_rtmpose', mode='tiny', display=True):
        ...
        while cap.isOpened():
            ...
            if not success:
                break
            # image_height is vertical length, and image_width is horizontal length
            image_height, image_width, _ = image.shape
            ...
            if keypoints is not None:
                ...
                # mean coordinates for hip
                cx_1 = int(np.mean([x[0] for x in landmarks_1]))
                cy_1 = int(np.mean([x[1] for x in landmarks_1]))
                self.landmark_1_buffer.push(cy_1)
                ...
                # x, y coordinates for elbows
                list_3_tmp = self.selected_landmarks_rtmlib_3
                landmarks_3 = [(keypoints[0, list_3_tmp[0], 0], keypoints[0, list_3_tmp[0], 1]),
                               (keypoints[0, list_3_tmp[1], 0], keypoints[0, list_3_tmp[1], 1])]
                # mean coordinates for elbows
                cy_3 = int(np.mean([x[1] for x in landmarks_3]))
                self.landmark_3_buffer.push(cy_3)

                # x, y coordinates for hands
                list_4_tmp = self.selected_landmarks_rtmlib_4
                landmarks_4 = [(keypoints[0, list_4_tmp[0], 0], keypoints[0, list_4_tmp[0], 1]),
                               (keypoints[0, list_4_tmp[1], 0], keypoints[0, list_4_tmp[1], 1])]
                # mean coordinates for hands
                cy_4 = int(np.mean([x[1] for x in landmarks_4]))
                self.landmark_4_buffer.push(cy_4)
                ...
                # x, y coordinates for feet
                list_5_tmp = self.selected_landmarks_rtmlib_5
                landmarks_5 = [(keypoints[0, list_5_tmp[0], 0], keypoints[0, list_5_tmp[0], 1]),
                               (keypoints[0, list_5_tmp[1], 0], keypoints[0, list_5_tmp[1], 1])]
                # mean coordinates for feet
                cy_5 = int(np.mean([x[1] for x in landmarks_5]))
                cy_hand_feet = cy_5 - cy_4
                self.landmark_5_buffer.push(cy_5)
            ...
            # two criteria for detecting a sub situp process
            self.bound_switch_up.push(cy_min + 0.35 * dy)
            self.bound_switch_down.push(cy_max - 0.55 * dy)
            ...
            if frame_num <= self.buffer_time:
                ...
                cv2.circle(img_show, (cx_2, cy), 5, (0, 0, 255), -1)
                cv2.putText(
                    img_show,
                    "centroid",
                    (cx_2 - 25, cy - 25),
    ...
    def plot_image_save(self, save_folder_path):
        ...
        and save the processed images in the SportsCounter/outputs folder.
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened():
                ...
                ax1.plot(self.cy_eh_buffer.buffer, label="cy_elbow_hand")
                ...
                ax1.plot(self.landmark_3_buffer.buffer, label="landmark_3")
                ax1.plot(self.landmark_4_buffer.buffer, label="landmark_4")
                ...
                ax1.plot(self.critirion_2.buffer, label="critirion_2")
                ...
                ax2.set_title("Video Frames and Pullup Count")
    ...
    def plot_image_save_rtmlib(self, save_folder_path, pose='local_rtmpose', mode='tiny'):
        ...
        while cap.isOpened():
            ...
            ax2.set_xlim(0, image_width)
            ax2.set_ylim(image_height, 0)
            ax2.set_title("Video Frames and Situp Count")
    ...
    def plot_image_save_AllLandmarks(self, save_folder_path, index_list):
        """
        Method for drawing posture preductions using mediapipe as model,
        and save the processed images in the SportsCounter/outputs folder.
    ...
    def plot_image_save_AllLandmarks_rtmlib(self, save_folder_path, index_list, pose='local_rtmpose', mode='tiny'):
        """
        Method for drawing posture preductions using rtmlib as model,
        and save the processed images in the SportsCounter/outputs folder.
