...
class Body:
    MODE = {
        ...
        'tiny': {
            'det':
            'models/yolox_nano.onnx',  # noqa
            'det_input_size': (416, 416),
            'pose':
            'models/end2end_tiny.onnx',  # noqa
            'pose_input_size': (192, 256),
        }
    }
    ...
    LOCAL_MODE_1 = {
        ...
        'small': {
            'det':
            'models/yolox_tiny.onnx',  # noqa
            'det_input_size': (416, 416),
            'pose':
            'models/end2end_small.onnx',  # noqa
            'pose_input_size': (192, 256),
        },
    }
    ...
    def __init__(self,
        ...
        elif pose is not None and 'local_rtmpose' in pose:
            from .. import YOLOX, RTMPose

            self.one_stage = False

            det = self.LOCAL_MODE_1[mode]['det']
            det_input_size = self.LOCAL_MODE_1[mode]['det_input_size']
            pose = self.LOCAL_MODE_1[mode]['pose']
            pose_input_size = self.LOCAL_MODE_1[mode]['pose_input_size']
        ...
        elif pose is not None and 'local_rtmo' in pose:
            from .. import RTMO

            self.one_stage = True

            pose = self.LOCAL_MODE_2[mode]['pose']
            pose_input_size = self.LOCAL_MODE_2[mode]['pose_input_size']
            self.pose_model = RTMO(pose,
                                   model_input_size=pose_input_size,
                                   to_openpose=to_openpose,
                                   backend=backend,
