...
# main class of implementing the situp counter
class SitupCounter:
    ...
    def __init__(self, file_name, activate_coef, buffer_time):
        ...
        # landmark indices for knees
        self.selected_landmarks_3 = [25, 26]
        self.selected_landmarks_rtmlib_3 = [13, 14]
        # landmark indices for feet
        self.selected_landmarks_4 = [29, 30]
        self.selected_landmarks_rtmlib_4 = [15, 16]
        # some default values
        self.cy_max = 100
        self.cy_min = 100
        ...
        self.knee_bottom_buffer = BufferList(buffer_time)
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened():
                ...
                if results.pose_landmarks:
                    ...
                    cy_shoulder_hip = np.sqrt((cx_1 - cx_2)**2 + (cy_1 - cy_2)**2)
                    self.shoulder_hip_buffer.push(cy_shoulder_hip)

                    # x, y coordinates for knees
                    landmarks_3 = [
                        (lm.x * image_width, lm.y * image_height)
                        for i, lm in enumerate(results.pose_landmarks.landmark)
                        if i in self.selected_landmarks_3
                    ]
                    # mean coordinates for knees
                    ...
                    self.landmark_3_buffer.push(cy_4)

                    cy_bottom = (cy_1 + cy_4)/2
                    cy_knee_bottom = cy_bottom - cy_3
                    self.knee_bottom_buffer.push(cy_knee_bottom)
                ...
                # detect whether the situp amplitude is large enough for the person
                ...
                if cy_knee_bottom >= 0.5 * cy_shoulder_hip:
                    leg_up = True
                else:
                    leg_up = False
                ...
                if flip_flag < prev_flip_flag:
                    ...
                    # update the count if there is a period of situp
                    count = count + 1
    ...
    def count_plot_rtmlib(self, pose='local_rtmpose', mode='tiny', display=True):
        ...
        while cap.isOpened():
            ...
            if keypoints.shape[0] > 1:
                coords_mat = np.zeros((keypoints.shape[0], 2))
                for q in range(keypoints.shape[0]):
                    coords_mat[q, 0] = keypoints[q, 11, 0] - image_width/2
                    coords_mat[q, 1] = keypoints[q, 12, 0] - image_width/2
            ...
            # detect whether the leg is in good posture
            if cy_knee_bottom >= 0.35 * cy_shoulder_hip:
                leg_up = True
            else:
                leg_up = False
